...
//json -> string -> js objet
const dataset = JSON.parse(JSON.stringify(require("../dataset/datasetOfPage.json")))
const{customtest} = require("../dataset/test-base")
 
 for( const data of dataset){
test(`clintAppPageObject ${data.email}`, async ({ page }) => {
   ...
   login.goTo();
   login.varifyLogin(data.email,data.password)
...
customtest('clintAppPageObject ghgheie ', async ({ page, loginInfo }) => {
   // const email = "anshika@gmail.com";
   // const password = "Iamking@000";
   ...
   login.goTo();
   login.varifyLogin(loginInfo.email,loginInfo.password)
